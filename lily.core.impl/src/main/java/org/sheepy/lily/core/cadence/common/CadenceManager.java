...
public class CadenceManager implements ICadenceManager
{
	...
	private List<IAllocator> engineAllocators;
	...
	public void load()
	{
		...
		engineAllocators = application.engines()
									  .stream()
	...
	public void run()
	{
		...
		while (stop.get() == false)
		{
			...
			for (final var engine : application.engines())
			{
	...
	private void dispose()
	{
		...
		((FeaturedObject<?>) application).disposeExtenderManager();
	...
	private static IAllocator allocateEngine(final IEngine engine)
	{
		final var allocator = IAllocationService.INSTANCE.buildAllocator(engine, null, IEngineAllocation.class);
	...
	{
		final var timeConfig = application.timeConfiguration();
		if (timeConfig != null)
		{
			final var unit = TimeUtil.resolveUnit(timeConfig.unit());
			final long step = (long) (timeConfig.timeStep() * timeConfig.timeFactor());
			assert unit != null;
