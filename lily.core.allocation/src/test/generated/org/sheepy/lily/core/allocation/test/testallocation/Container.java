...
public interface Container extends AllocationObject {
  ...
  interface Features<T extends Features<T>> extends AllocationObject.Features<T> {
    ...
    Relation<Box, List<Box>, Listener<List<Box>>, Features<?>> BOXES = new RelationBuilder<Box, List<Box>, Listener<List<Box>>, Features<?>>().name("boxes").many(true).contains(true).id(Container.FeatureIDs.BOXES).concept(() -> TestallocationModelDefinition.Groups.BOX).build();
    List<Feature<?, ?, ?, ?>> ALL = List.of(CURRENT_ALLOCATION_COUNT, TOTAL_ALLOCATION_COUNT, DEPENDENCY_UPDATE_COUNT, ACTIVATED, BOXES);
  }

  interface Builder extends IFeaturedObject.Builder<Container> {
