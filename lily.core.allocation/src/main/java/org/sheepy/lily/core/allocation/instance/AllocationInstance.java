...
public final class AllocationInstance<Allocation extends IAdapter>
{
	...
	public AllocationInstance(final IFeaturedObject target,
							  ...
							  final IAdapterDescriptor.ExtenderContext<Allocation> extenderContext)
	{
		...
		if (observatory != null)
		{
			if (!(target instanceof LMObject lmObject))
			{
				throw new IllegalArgumentException("Allocation target must be an LMObject");
			}
			observatory.observe(lmObject);
		}
	}
	...
	public void free(final IFeaturedObject target, final IAllocationContext context)
	{
		assert isUnlocked();
		if (!(target instanceof LMObject lmObject))
		{
		...
		if (preObservatory != null) preObservatory.shut(lmObject);
		if (observatory != null) observatory.shut(lmObject);
