...
public final class ChildrenSupervisor
{
	...
	public void update(final IFeaturedObject source)
	{
		if (addedElements)
		{
			addedElements = false;

			final var children = modelExplorer.stream((LMObject) source)
											  .toList();
	...
	public void postUpdate(final IFeaturedObject source)
	{
		for (final var injector : injectors)
		{
			injector.inject(modelExplorer.stream((LMObject) source));
		}
	}

	@SuppressWarnings("MethodOnlyUsedFromInnerClass")
	private void add(List<LMObject> children)
	{
	...
	@SuppressWarnings("MethodOnlyUsedFromInnerClass")
	private void remove(List<LMObject> removedChildren)
	{
		for (final var childrenAllocator : childrenAllocators)
		{
	...
	private void reloadDescriptors(final List<LMObject> children)
	{
		children.stream()
