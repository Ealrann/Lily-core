package org.sheepy.lily.core.model.types;

import java.util.List;
import org.logoce.lmf.core.api.model.IFeaturedObject;
import org.logoce.lmf.core.api.model.IModelNotifier;
import org.logoce.lmf.core.api.notification.listener.FloatListener;
import org.logoce.lmf.core.lang.Attribute;
import org.logoce.lmf.core.lang.Feature;
import org.logoce.lmf.core.lang.LMCoreModelDefinition;
import org.logoce.lmf.core.lang.builder.AttributeBuilder;
import org.sheepy.lily.core.model.types.builder.FloatParameterBuilder;

public interface FloatParameter extends NativeParameter<Float> {
  static Builder builder() {
    return new FloatParameterBuilder();
  }

  @Override
  IModelNotifier<? extends Features<?>> notifier();
  float value();
  void value(final float value);
  Float getNativeValue();

  interface FeatureIDs {
    int VALUE = 2126210599;
  }

  interface Features<T extends Features<T>> extends NativeParameter.Features<T> {
    Attribute<Float, Float, FloatListener, Features<?>> VALUE = new AttributeBuilder<Float, Float, FloatListener, Features<?>>().name("value").id(FloatParameter.FeatureIDs.VALUE).datatype(() -> LMCoreModelDefinition.Units.FLOAT).build();
    List<Feature<?, ?, ?, ?>> ALL = List.of(VALUE);
  }

  interface Builder extends IFeaturedObject.Builder<FloatParameter> {
    Builder value(float value);
  }
}
