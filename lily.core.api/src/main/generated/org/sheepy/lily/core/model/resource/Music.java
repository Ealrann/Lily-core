...
public interface Music extends IRootResource {
  ...
  interface Features<T extends Features<T>> extends IRootResource.Features<T> {
    ...
    Relation<FileResource, FileResource, Listener<FileResource>, Features<?>> FILE = new RelationBuilder<FileResource, FileResource, Listener<FileResource>, Features<?>>().name("file").mandatory(true).contains(true).id(Music.FeatureIDs.FILE).concept(() -> ResourceModelDefinition.Groups.FILE_RESOURCE).build();
    List<Feature<?, ?, ?, ?>> ALL = List.of(NAME, FILE);
  }

  interface Builder extends IFeaturedObject.Builder<Music> {
